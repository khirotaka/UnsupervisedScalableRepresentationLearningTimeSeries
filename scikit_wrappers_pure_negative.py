...
from advanced.triplet_loss import TripletLossPureNegative
...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def __init__(self, compared_length, nb_random_samples, negative_penalty,
        ...
        self.loss = TripletLossPureNegative(compared_length, nb_random_samples, negative_penalty)
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False):
        ...
        while i < self.nb_steps:
            ...
            for batch in train_generator:
                if self.cuda:
                    batch = tuple(map(lambda x: x.to(self.gpu) if isinstance(x, torch.Tensor) else x, batch))
                self.optimizer.zero_grad()
                if not varying:
                    loss = self.loss(
                        batch, self.encoder, train, train_torch_dataset.labels
                    )
                else:
